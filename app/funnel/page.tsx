...
const reviews = [
  ...
  { name: "Sam Brown", label: "PT Launch Lab Student", quote: "Callum and Ryan helped me so much with not only the course but with any other challenges I was unsure on. Nothing was too much to ask — both really patient and understanding. Could not recommend enough." },
...
export default function FunnelPage() {
  ...
  async function handleProspectusSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Open PDF in new tab then close modal
  ...
  return (
    <div className="bg-[#072B4A] font-[Poppins,sans-serif]">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md bg-[#0D3559] border border-[#3B82F6]/30 rounded-2xl p-8 shadow-2xl">
            ...
            <button
              ...
              className="absolute top-4 right-4 text-[#8CA3BF] hover:text-white transition-colors text-xl font-bold"
              aria-label="Close"
            >
              ✕
            </button>
            ...
            <div className="mb-6">
              ...
              <p className="text-[#8CA3BF] text-sm leading-relaxed">
                Enter your details below and we&apos;ll take you straight to the prospectus — full course overview, pricing, and what to expect.
              </p>
            </div>
            ...
            <form onSubmit={handleProspectusSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setProspectusForm((f) => ({ ...f, name: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setProspectusForm((f) => ({ ...f, phone: e.target.value }))}
                  className="w-full bg-[#072B4A] border border-[#3B82F6]/30 rounded-xl px-4 py-3 text-white placeholder-[#4A6280] text-sm focus:outline-none focus:border-[#F5C518]/60 transition-colors"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setProspectusForm((f) => ({ ...f, email: e.target.value }))}
                  className="w-full bg-[#072B4A] border border-[#3B82F6]/30 rounded-xl px-4 py-3 text-white placeholder-[#4A6280] text-sm focus:outline-none focus:border-[#F5C518]/60 transition-colors"
                />
              </div>

              {prospectusError && (
                <p className="text-red-400 text-sm">{prospectusError}</p>
              )}

              <button
                type="submit"
                disabled={prospectusSubmitting}
                className="w-full py-4 rounded-full bg-[#F5C518] text-[#072B4A] font-bold text-base hover:brightness-110 transition-all disabled:opacity-60 disabled:cursor-not-allowed mt-2"
              >
      ...
      {/* ── TOP BAR ─────────────────────────────────────────── */}
      <div className="bg-[#051D33] border-b border-[#3B82F6]/15 px-6 py-3 grid grid-cols-3 items-center">
        {/* Left — logo */}
        ...
        <nav className="flex items-center justify-center gap-2 sm:gap-3">
          <a
            href="/enrol"
            className="px-5 py-2 rounded-full bg-[#F5C518] text-[#072B4A] text-sm font-bold hover:brightness-110 transition-all shadow-md shadow-[#F5C518]/20"
          >
            Start Today →
          </a>
          <a
            href="/book-call"
            className="px-4 py-2 rounded-full border border-white/20 text-sm font-semibold text-white hover:bg-white/10 transition-all"
          >
            Book a Call
          </a>
          <a
            href="#course-info"
            className="px-4 py-2 rounded-full text-sm font-semibold text-[#8CA3BF] hover:text-white hover:bg-white/5 transition-all"
          >
            Course Info
          </a>
        </nav>
        ...
        <div className="flex justify-end">
          <a href="/" className="text-xs text-[#4A6280] hover:text-[#8CA3BF] transition-colors font-medium">
            ← Main site
          </a>
        </div>
      </div>
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-[#F5C518]/40 bg-white/10 backdrop-blur-sm mb-8">
                <span className="text-[#F5C518] text-xs font-semibold tracking-widest uppercase">
                  Level 2 &amp; 3 PT · NCFE · Ofqual · CIMSPA · REPs
                </span>
              </div>
              ...
              <div className="mb-8">
                <a
                  href="/quiz"
                  className="inline-flex items-center gap-2 text-blue-200/70 text-sm font-medium hover:text-white transition-all"
                >
              ...
              {/* Trust bar */}
              <div className="flex flex-wrap gap-3 md:gap-5 text-blue-200/70 text-xs">
      ...
      <section className="bg-[#072B4A] py-24">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {painCards.map((card) => (
              <div key={card.title} className="bg-[#0D3559] border border-[#3B82F6]/25 rounded-2xl p-8 hover:border-[#F5C518]/60 transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                <a
                  href="/enrol"
                  className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-[#F5C518] text-[#072B4A] font-bold hover:brightness-110 transition-all"
                >
                  Start your journey today →
                </a>
                <a
                  href="/quiz"
                  className="inline-flex items-center gap-2 px-8 py-4 rounded-full border border-[#F5C518]/40 text-[#F5C518] font-semibold text-sm hover:bg-[#F5C518]/10 transition-all"
                >
      ...
      <section className="bg-[#051D33] py-24">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-14">
            {steps.map((step) => (
              <div key={step.num} className={`rounded-2xl p-8 border transition-all ${step.highlight ? "bg-[#0D3559] border-[#F5C518]/60 shadow-xl shadow-[#F5C518]/10" : "bg-[#0D3559] border-[#3B82F6]/25"}`}>
                <div className="flex items-center gap-4 mb-6">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ${step.highlight ? "bg-[#F5C518]/15 border border-[#F5C518]/40" : "bg-[#072B4A] border border-[#3B82F6]/25"}`}>
                    {step.icon}
                  </div>
                  <div>
      ...
      <section id="course-info" className="bg-[#0D3559] py-24">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-14">
            {features.map((feat) => (
              <div key={feat.title} className="bg-[#072B4A] border border-[#3B82F6]/25 rounded-2xl p-7 hover:border-[#F5C518]/40 transition-all duration-300">
          ...
          {/* Prospectus CTA */}
          <div className="flex flex-col items-center gap-4 bg-[#072B4A] border border-[#F5C518]/25 rounded-2xl p-8 text-center">
            <span className="text-[#F5C518] text-xs font-bold tracking-widest uppercase">Free Download</span>
            <h3 className="text-white text-2xl font-bold">Want the full course breakdown?</h3>
            <p className="text-[#8CA3BF] text-sm max-w-md leading-relaxed">
              Download our free course prospectus — full module list, pricing options, and exactly what to expect from day one.
            </p>
            <button
              onClick={() => setProspectusOpen(true)}
              className="mt-2 px-10 py-4 rounded-full bg-[#F5C518] text-[#072B4A] font-bold text-base hover:brightness-110 transition-all shadow-lg shadow-[#F5C518]/20"
            >
              📄 Download Free Prospectus →
            </button>
            <p className="text-[#4A6280] text-xs">No spam. We&apos;ll only reach out if you&apos;d like us to.</p>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
            <div>
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                <a href="https://www.google.com/search?q=pt+launch+lab+pontefract+reviews" target="_blank" rel="noopener noreferrer" className="text-[#F5C518] text-sm font-medium hover:underline ml-1">
                  See all →
                </a>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={prevReview} disabled={reviewIndex === 0} aria-label="Previous review" className="w-11 h-11 rounded-full border border-[#3B82F6]/40 flex items-center justify-center text-white hover:border-[#F5C518] hover:text-[#F5C518] transition-all disabled:opacity-30 disabled:cursor-not-allowed">‹</button>
              <button onClick={nextReview} disabled={reviewIndex >= maxReviewIndex} aria-label="Next review" className="w-11 h-11 rounded-full border border-[#3B82F6]/40 flex items-center justify-center text-white hover:border-[#F5C518] hover:text-[#F5C518] transition-all disabled:opacity-30 disabled:cursor-not-allowed">›</button>
            </div>
          </div>
          <div
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-[#0D3559] border-2 border-[#F5C518]/80 rounded-2xl p-8 text-left flex flex-col shadow-xl shadow-[#F5C518]/10">
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {["Immediate access on enrolment", "Start your first module today", "Your personal tutor assigned from day one", "Study at your own pace"].map((item) => (
      ...
      <section className="bg-gradient-to-br from-[#0E5FA0] via-[#0A4A80] to-[#072B4A] py-28 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6">
          ...
          <p className="text-blue-200/50 text-xs">No commitment required. Start within minutes of enrolling.</p>
        </div>
      </section>
      ...
      <div className="bg-[#051D33] border-t border-[#3B82F6]/15 px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <a href="/" className="text-sm text-[#8CA3BF] hover:text-white transition-colors font-medium">
          ← Visit main site
        </a>
      </div>
