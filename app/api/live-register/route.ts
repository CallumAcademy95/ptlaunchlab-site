...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (webhookUrl) {
      ...
      // Resilient push — a Zapier blip must not 500 the registration or lose
      // the lead. Retry once, then log loudly (search `level:lead-lost`).
      ...
      console.warn('[live-register] no webhook configured — skipping.');
    }
