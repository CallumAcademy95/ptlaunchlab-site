// scripts/upload-gym-promo-packs.mts
/**
 * Put each gym's monthly promo pack into their resource drive.
 *
 *   npx tsx scripts/upload-gym-promo-packs.mts               # dry run
 *   npx tsx scripts/upload-gym-promo-packs.mts --apply        # upload
 *   npx tsx scripts/upload-gym-promo-packs.mts --apply --replace
 *                                                           # let a reissued
 *      graphic overwrite the resource of the same title, instead of being
 *      skipped as already present.
 *
 * Sibling of upload-gym-ad-packs.mts — same job, same guards, different
 * source directory, storage prefix, title and `pack` field. Read that
 * script's header before touching this one; nothing below repeats reasoning
 * that already lives there in full.
 *
 ...
 * `${slug}/promos/`, checked immediately before the PATCH. A title match
 ...
 * the literal `Promo — <label> (<shape>)` string built a few lines down —
 ...
 * object: the storage path for a given (slug, month, size) is always the
 ...
 *
 * `pack` is set to the month's playbook campaign slug so a partner reads the
 * campaign entry in their Playbook and finds the matching graphics
 * underneath it, rather than meeting 40 files in one flat resource list.
 *
...
const { SIZES } = await import("./lib/ad-concepts.mjs");
const { MONTHS } = await import("./lib/promo-calendar.mjs");
...
for (const slug of GYM_SLUGS) {
  ...
  for (const [monthIndex, month] of MONTHS.entries()) {
    ...
    for (const { w, h } of SIZES) {
      ...
      const objectPath = `${slug}/promos/${file}`;
      ...
      // live under this gym's promos/ prefix, so it wasn't created by this
      // script. Never patch it — surface it and move on.
      if (prior && REPLACE && !prior.storage_path?.startsWith(`${slug}/promos/`)) {
        console.warn(
          `!! COLLISION  ${slug.padEnd(16)} "${title}" already exists (id ${prior.id}, storage_path "${prior.storage_path}") ` +
            `but that path isn't under ${slug}/promos/ — left untouched, not patched. Needs a human to look at it.`
        );
        skipped++;
        continue;
      }
